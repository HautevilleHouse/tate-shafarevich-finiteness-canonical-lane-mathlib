/-!
# Source package model for `tate-shafarevich-finiteness-canonical-lane`
...
def sourceCheckoutHead : String := "042dfbdf91c5ff1b37b9c29b1b559234d240cce7"
...
def sourceFiles : List SourceFile := [
  ...
  { path := "scripts/tsf_closure_guard.py", sha256 := "1e952902b64536c806dd621a45a9ebc96a3be0872c6e27eb3b6c7a4c7377b558", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  ...
  { file := "scripts/tsf_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 48, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 60, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 77, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 88, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 121, isAsync := false },
  { file := "scripts/tsf_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 128, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  ...
  { file := "scripts/tsf_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/tsf_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/tsf_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/tsf_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/tsf_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/tsf_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/tsf_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_descent\", \"sigma_selmer\", \"kappa_compact\", \"rho_rigidity\", \"sha_transfer\"]", line := 21 },
  { file := "scripts/tsf_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_descent", status := "derived_numeric", formula := "c_descent_raw * descent_density_raw - e_descent_raw", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["c_descent_raw", "descent_density_raw", "e_descent_raw"] },
  { group := "constants", key := "sigma_selmer", status := "derived_numeric", formula := "selmer_floor_raw - local_loss_raw - restart_loss_raw", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["local_loss_raw", "restart_loss_raw", "selmer_floor_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "sha_transfer", status := "derived_numeric", formula := "c_sha_raw * transfer_gain_raw - e_sha_raw", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_sha_raw", "e_sha_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Appendix E.6" },
  { key := "kappa_compact", value := "0.8051529790660226", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "kappa_descent", value := "1.0922840000000003", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_descent", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "rho_rigidity", value := "1.076", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sha_transfer", value := "1.029422", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sha_transfer", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "sigma_selmer", value := "1.074", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_selmer", sourceSection := "paper/TATE_SHAFAREVICH_FINITENESS_PREPRINT.md Section 5.2 / Appendix B" }
]
